...
function mainButtonFunction() {
  ...
  if (button_state == 2) {
    if(Math.abs(parseInt(range_button.value)) + 1 && range_button.value > min_num){
      ...
      else if (range_button.value <= min_num) {
        alert("El número debe ser mayor a " + min_num)
        button_state = 1;
      }
    }
  }
  ...
  if (button_state == 5) {
    ...
    if (parseInt(change1.value) < parseInt(change2.value) && Math.abs(parseInt(change1.value)) + 1 && Math.abs(parseInt(change2.value)) + 1) {
      min_num = parseInt(change1.value);
      max_num = parseInt(change2.value);
      if (!terminate)
        lot_val = 0;
      else if (terminate)
        lot_val = cycle_times;
      ...
      if (parseInt(change1.value) >= parseInt(change2.value)) {
        alert("El número de la izquierda debe ser menor a " + change2.value);
      }
